...
export function filterTasks<T extends Uffgabe>(
  ...
  return tasks.filter(task => {
    ...
    if (criteria.assigneeId !== undefined && task.assignee?.id !== criteria.assigneeId) {
      return false;
    }
    if (criteria.dueBefore !== undefined && (!task.dueDate || task.dueDate >= criteria.dueBefore)) {
      return false;
    }
    if (criteria.dueAfter !== undefined && (!task.dueDate || task.dueDate <= criteria.dueAfter)) {
      return false;
    }
    return true;
  });
 ...
 */
export function summarizeTasksByStatus<T extends Uffgabe>(tasks: T[]): Record<TaskStatus, number> {
  const result = {
 ...
 */
export function formatTask(task: Uffgabe & { status: TaskStatus.PENDING }): string;
 ...
 * @returns Formatted string
 */
export function formatTask(task: Uffgabe & { status: TaskStatus.LAEUFT }): string;
 ...
 * @returns Formatted string
 */
export function formatTask(task: Uffgabe & { status: TaskStatus.FERTIG | TaskStatus.MIST }): string;
